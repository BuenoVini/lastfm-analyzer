...
class PlotterFM():
    ...
    def __render_barh(self, data: np.ndarray, title: str, xlabel: str, ylabel: str) -> None:
        ...
        plt.barh(y=self.__months_names[::-1], width=data[::-1])
    ...
    def total_artists_year(self, year: str) -> None:
        total_artists = np.array( [self.__analyzer.highlights_month(f'{year}-{month}').total_artists for month in range(1, 13)] )
    ...
    def total_scrobbles_year(self, year: str) -> None:
        total_scrobbles = np.array( [self.__analyzer.highlights_month(f'{year}-{month}').total_scrobbles for month in range(1, 13)] )
